...
<script>
	$(document).ready(function() {
		...
		$('input:radio[name=category]').eq(Math.abs(category)).attr('checked','checked');
		});
	function category() {
...
<span>주문상태 카테고리</span>
	<form action="/admin/managechange?pn=1" method="post" id="categorySubmit">
		<label>
			<input type="radio" name="category" value="0" checked="checked"> 전체
		</label>
		<label>
			<input type="radio" name="category" value="-1"> 결제취소
		</label>
		<label>
			<input type="radio" name="category" value="-2"> 환불요청
		</label>
		<label>
			<input type="radio" name="category" value="-3"> 환불완료
		</label>
		<label>
			<input type="radio" name="category" value="-4"> 교환신청
		</label>
		<label>
			<input type="radio" name="category" value="-5"> 교환완료
		</label>
	</form>
...
<table class="table table-dark table-striped table-hover" id="table">
	...
	<tr onclick="showContext(this)" style="cursor:pointer;">
		...
		<td>
		<form action="/admin/updatesttus?pn=<%=pagenation.getStartPageNumber()%>&category=<%=category%>" method="post" id="updateSttus">
		<% if (list.get(i).getSttus() == -2 || list.get(i).getSttus() == -4) {
			switch(list.get(i).getSttus()) {
			case -2 : %>
			<label>
			<input type="radio" name="orderSttus" value="-5">환불완료
			</label>
			<button class="btn btn-primary" onclick="updateOrderSttus()">변경</button>
		<%	break;
			case -4: %>
			<label>
			<input type="radio" name="orderSttus" value="-3">교환완료
			</label>
			<br>
			<label>
			<input type="radio" name="orderSttus" value="-5">환불완료
	...
	<div>
		...
		<% if (pagenation.getStartPageNumber() != 1) { %>
			<li class="page-item"><a class="page-link" href="/admin/managechange?pn=<%=pagenation.getStartPageNumber() - 1%>&category=<%=category%>"> << </a></li>
		<% }%>
		<% for ( int i = pagenation.getStartPageNumber(); i <= pagenation.getEndPageNumber(); i++) {
			if (i != Integer.parseInt(pagenum)) {
			%> 
			<li class="page-item"><a class="page-link" href="/admin/managechange?pn=<%=i%>&category=<%=category%>"><%=i%></a></li>
			<% } else { %> 
		...
		<li class="page-item"><a class="page-link" href="/admin/managechange?pn=<%=pagenation.getEndPageNumber() + 1%>&category=<%=category%>"> >> </a></li>
		<% } %>
		</ul>
	</div>
</section>
