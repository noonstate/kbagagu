...
public class DeleteNoticeAction implements Action {
	...
	public ActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (!RegExp.isValidExp(pagenum, REGEXP_NUMBER)) {
			response.setContentType("text/html;charset=UTF-8");
			PrintWriter out = response.getWriter();
			out.println("<script>location.href='/admin/deletenotice?pn=1';</script>");
			out.close();
			return null;
		}
		...
		if (page > pagenation.getTotalPageCount()) {
			...
			PrintWriter out = response.getWriter();
			out.println(
					"<script>location.href='/admin/deletenotice" + pagenation.getTotalPageCount() + "';</script>");
			out.close();
		}
		...
		forward.setPath("/views/admin/deletenotice.jsp");
		return forward;
